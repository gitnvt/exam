...
use App\Components\Activitylog\Handlers\ActivitylogHandlerInterface;
use App\Components\Activitylog\Handlers\DefaultLaravelHandler;
...
class ActivitylogSupervisor
{
     ...
     * @param Guard                                $auth
     */
    public function __construct(ActivitylogHandlerInterface $logHandler, Repository $config, Guard $auth)
    {
        $this->config = $config;
    ...
    public function log($text, $userId = '', $attributes=[])
    {
        ...
        foreach ($this->logHandlers as $logHandler) {
            $logHandler->log($text, $userId, $attributes);
        }
